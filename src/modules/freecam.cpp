...
void Freecam::ApplyToOrigin(SDK::Vec3& renderOrigin, SDK::Vec3& viewTarget) {
    ...
    // Movement direction from the camera's own facing (target - origin), so WASD
    // is relative to where you're looking. No LocalPlayer needed.
    ...
    // Shift both vectors by the delta from the real origin to freePos
