...
bool Freecam::WantsOverride(SDK::CameraComponent* cam) {
    if (!g_freecam || !g_freecam->enabled || !cam) return false;

    // Initialize + advance movement here (runs once per camera update)
...
SDK::Vec3 Freecam::GetOverridePos(SDK::CameraComponent*) {
    return g_freecam ? g_freecam->freePos : SDK::Vec3{ 0, 0, 0 };
}
