...
namespace SDK {
    // Render camera chain — verified via Ghidra (FUN_14314eac0, the inner renderLevel).
    // LevelRendererPlayer holds TWO Vec3s:
    //   0x654 = render origin (X/Y/Z) — what chunk/block rendering + culling + distance
    //           sorting reads. THIS is the true camera position for the world.
    //   0x660 = view/look target point — used for the view direction vector.
    // Latite's getOrigin was 0x660 (the target), which is why writing it only moved
    // entities/particles and left blocks anchored to the real player. The block origin
    // is 0x654.
    class LevelRendererPlayer {
    public:
        Vec3& getRenderOrigin() { return member_at<Vec3>(this, 0x654); }  // blocks + world
        Vec3& getViewTarget()   { return member_at<Vec3>(this, 0x660); }  // look direction
    };
