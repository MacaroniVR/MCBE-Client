...
namespace SDK {
    ...
    //   0x6E4 = render camera pos (X/Y/Z) — VERIFIED for 1.21.5X via Flarial's
    //           offset DB (LevelRendererPlayer::cameraPos). The old 0x654 was from
    //           the 26.45 dump and is wrong for 1.21.51 — that's why blocks never
    //           moved. View target sits right after at 0x6F0.
    class LevelRendererPlayer {
    public:
        Vec3& getRenderOrigin() { return member_at<Vec3>(this, 0x6E4); }  // blocks + world
        Vec3& getViewTarget()   { return member_at<Vec3>(this, 0x6F0); }  // look direction
    };
