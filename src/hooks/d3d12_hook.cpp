...
namespace D3D12Hook {
    ...
    static LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
        ...
        if (g_menuOpen) {
            ...
            // Block input from reaching the game while menu is open
            switch (msg) {
                case WM_MOUSEMOVE:
    ...
    static bool SetupRenderer(IDXGISwapChain3* swapchain) {
        ...
        // Load a crisp font at a good size (Segoe UI ships on all Windows)
        {
            ...
            // Glyph ranges: default Latin + the arrows block (U+2190-U+27FF)
            // so symbols like the reset glyph render. Must be static (ImGui keeps
            // the pointer and reads it at build time).
            static const ImWchar ranges[] = {
                0x0020, 0x00FF,   // Basic Latin + Latin-1 Supplement
                0x2190, 0x21FF,   // Arrows
                0x27F0, 0x27FF,   // Supplemental Arrows-A (contains ⟳ U+27F3)
                0x2B00, 0x2BFF,   // Misc symbols and arrows (spare)
                0,
            };

            ImFont* f = io.Fonts->AddFontFromFileTTF(fontPath.c_str(), 20.0f, &cfg, ranges);
            if (!f) {
                // Fallback to default if Segoe not found
                io.Fonts->AddFontDefault();
                std::cout << "[!] Segoe UI not found, using default font\n";
            }
        }
    ...
    static HRESULT __stdcall hkPresent(IDXGISwapChain3* swapchain, UINT syncInterval, UINT flags) {
        ...
        // Always-on overlays + keybind polling (independent of menu being open)
        Menu::Poll();
        FpsCounter::Draw();

        if (g_menuOpen) {
            // Keep the OS cursor visible — Bedrock tries to hide it every frame
            ImGuiIO& io = ImGui::GetIO();
            io.MouseDrawCursor = true;

            // Un-clip the cursor so it can leave the center of the screen
            ClipCursor(nullptr);
