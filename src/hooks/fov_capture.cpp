...
namespace FovCapture {
    ...
    bool Init() {
        ...
        // Allocate a code cave near the target (within +-2GB for rel32 jmps)
        g_trampoline = (BYTE*)VirtualAlloc(nullptr, 64, MEM_COMMIT | MEM_RESERVE,
                                           PAGE_EXECUTE_READWRITE);
        if (!g_trampoline) {
            std::cout << "[FovCapture] cave alloc failed\n";
            return false;
        }
        ...
        *p++ = 0xE9;
        int32_t rel = (int32_t)(backTarget - ((uintptr_t)p + 4));
        memcpy(p, &rel, 4); p += 4;
        ...
        w[0] = 0xE9;
        int32_t relToCave = (int32_t)((uintptr_t)g_trampoline - (g_writeAddr + 5));
