...
namespace CameraHook {
    ...
    static void* __fastcall hkBuildCamera(void* result, void* p2, void* p3, void* p4) {
        ...
        // Only override when freecam is on
        if (g_freecam && g_freecam->enabled && g_freecam->posInitialized) {
            // camera object is at result[0x15] (byte offset 0xA8)
            ...
            if (cam && !IsBadWritePtr((void*)(cam + 0x28), 0x18)) {
                ...
                // Preserve the look DIRECTION: shift the view target by the same
                // delta as the position so the camera keeps facing the same way.
    ...
    bool Init() {
        // DISABLED: this hook used camera-object offsets from the old 26.45 dump
        // which are wrong for 1.21.51 and risk writing garbage. The real full-freecam
        // fix is the view matrix (GameRenderer+0x358) — see memory notes. Kept for
        // reference but not installed.
        std::cout << "[*] CameraHook: disabled (using origin-write path)\n";
        return true;
        /*
